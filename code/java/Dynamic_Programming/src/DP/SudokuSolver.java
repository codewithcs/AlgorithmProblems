...
public class SudokuSolver {
	...
	private static final char EMPTY_ENTRY = '.';
	  ...
	  private static boolean canSolveSudokuFromCell(int row, int col, char[][] board) {
	    ...
	    if (col == board[row].length) { // Base Case
	    ...
	    for (int value = 1; value <= board.length; value++) { // One by one, check for all values which can be placed on the empty cell. 
	      ...
	      if (canPlaceValue(board, row, col, charToPlace)) {
	        ...
	        board[row][col] = EMPTY_ENTRY; // backtrack if we can't place after col. 
	  ...
	  private static boolean canPlaceValue(char[][] board, int row, int col, char charToPlace) {
	    ...
	    for (char[] placementRow: board) {       // We can get individual rows of a 2d array like this.                
